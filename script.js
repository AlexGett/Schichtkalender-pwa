...
// --- FUNKTION ZUR ANZEIGE DER UPDATE-BENACHRICHTIGUNG (NEU) ---
/**
 * Erstellt und zeigt ein Pop-up/Banner an, um den Benutzer über ein Update zu informieren.
 * Sendet die 'SKIP_WAITING' Nachricht an den Service Worker.
 * @param {ServiceWorker} waitingWorker - Der neue, wartende Service Worker
 */
function showUpdateNotification(waitingWorker) {
	// Falls das Banner bereits existiert, nichts tun
    ...
    `;
	banner.innerHTML = '✨ Neues Update verfügbar! **Hier klicken** zum Aktualisieren.';

	banner.onclick = () => {
		// 1. Sende die Nachricht an den wartenden SW, um ihn sofort zu aktivieren
		waitingWorker.postMessage({ type: 'SKIP_WAITING' });

		// 2. Warten auf die Übernahme der Kontrolle (Control) durch den neuen SW
		// Wird auch durch den 'controllerchange'-Listener unten im DOMContentLoaded behandelt
		navigator.serviceWorker.oncontrollerchange = () => {
			// 3. Wenn der neue SW die Kontrolle hat, die Seite neu laden
			window.location.reload();
		};

		// Verstecke das Banner, während auf die Aktivierung gewartet wird
		banner.style.opacity = '0';
	};

	document.body.appendChild(banner);
}
// --- ENDE FUNKTION UPDATE-BENACHRICHTIGUNG ---


// Initialisierung bei DOMContentLoaded
document.addEventListener('DOMContentLoaded', () => {

	// *** NEU: Service Worker Registrierung & Update-Logik ***
	if ('serviceWorker' in navigator) {
		// Da Ihr Skript die Kalendergenerierung im 'DOMContentLoaded'-Event startet,
		// verwenden wir hier 'load', um sicherzustellen, dass die Seite fertig ist.
		// Für PWAs ist 'load' oft die sicherere Wahl für die SW-Registrierung.
		window.addEventListener('load', () => {
			// Pfad zu Ihrem Service Worker, meist '/sw.js'
			navigator.serviceWorker.register('/sw.js')
				.then(registration => {
					console.log('Service Worker registriert:', registration);

					// Listener, der ausgelöst wird, wenn der Browser eine neue SW-Datei findet
					registration.addEventListener('updatefound', () => {
						const installingWorker = registration.installing;
						console.log('Neuer Service Worker wird installiert...');

						installingWorker.addEventListener('statechange', () => {
							// Wenn der neue Worker installiert wurde, aber noch auf seine Aktivierung wartet
							if (installingWorker.state === 'installed' && registration.waiting) {
								// Ein Update ist verfügbar! Zeige die Benachrichtigung an
								console.log('Update gefunden. Zeige Benachrichtigung.');
		...
		// Zusätzliche Logik: Stellt sicher, dass die Seite sofort neu lädt, wenn der SW die Kontrolle übernimmt
		let refreshing = false;
		navigator.serviceWorker.addEventListener('controllerchange', () => {
			if (refreshing) return;
			// Wenn der Controller wechselt (neuer SW hat die Kontrolle übernommen), lade die Seite neu
			window.location.reload();
			refreshing = true;
		});
	}
