...
function generateCalendar(year) {
	...
	// **********************************************
	// NEU: Heutiges Datum für die Hervorhebung abrufen
	// **********************************************
	...
	for (let month = 0; month < 12; month++) {
		...
		for (let day = 1; day <= daysInMonth(year, month); day++) {
			...
			// **********************************************
			// NEU: Klasse .today-cell zuweisen, falls es heute ist
			// **********************************************
...
document.addEventListener('DOMContentLoaded', () => {
	...
	// **********************************************
	// NEU: Kennzeichnung des heutigen Tages beim Laden
	// **********************************************
	generateCalendar(currentCalendarYear); // Initialer Kalenderaufbau

	// Nach dem Generieren des Kalenders, das Today-Icon beat-animieren,
	// wenn das aktuelle Jahr angezeigt wird
	const today = new Date();
	const todayFormatted = today.getFullYear().toString() + '-' +
		(today.getMonth() + 1).toString().padStart(2, '0') + '-' +
		today.getDate().toString().padStart(2, '0');

	const todayIcon = document.getElementById('todayButton');
	if (todayIcon) {
		if (today.getFullYear() === currentCalendarYear) {
			todayIcon.classList.add('beat-animation');
		} else {
			todayIcon.classList.remove('beat-animation');
		}
	}
	// **********************************************
});
...
function saveCustomShiftSystem() {
	...
	// **********************************************
	// NEU: Aktualisiere die Animation nach dem Speichern
	// **********************************************
	const today = new Date();
	const todayIcon = document.getElementById('todayButton');
	if (todayIcon) {
...
function resetCustomShiftSystem() { // Name der Funktion bleibt gleich, aber die Funktionalität ändert sich
	...
	// **********************************************
	// NEU: Aktualisiere die Animation nach dem Zurücksetzen
	// **********************************************
	const today = new Date();
	const todayIcon = document.getElementById('todayButton');
	if (todayIcon) {
...
if (todayButton) {
	todayButton.addEventListener('click', () => {
		...
		// **********************************************
		// NEU: Sicherstellen, dass die Beat-Animation aktiv ist
		// **********************************************
		const todayIcon = document.getElementById('todayButton');
		if (todayIcon) {
			todayIcon.classList.add('beat-animation');
...
setYearButton.addEventListener('click', () => {
	...
	if (!isNaN(newYear) && newYear >= 2025 && newYear <= 2030) {
		...
		// **********************************************
		// NEU: Beat-Animation des Today-Icons anpassen
		// **********************************************
		const today = new Date();
		const todayIcon = document.getElementById('todayButton');
		if (todayIcon) {
			if (today.getFullYear() === newYear) {
				todayIcon.classList.add('beat-animation');
			} else {
				todayIcon.classList.remove('beat-animation');
			}
